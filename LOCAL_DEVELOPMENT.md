...
### 4. **Configure Web App (Optional)**

The deployment script automatically creates `apps/web/.env.local` with all contract addresses.

To enable wallet connections, set your WalletConnect Project ID:

```bash
# Edit the generated file
nano apps/web/.env.local

# Add or update your WalletConnect Project ID (get from cloud.walletconnect.com)
...
| **Setup Env**  | `cd apps/core && pnpm setup-web-env`  | Create .env.local interactively |
...
If you need to manually configure the web app environment:
...
This interactive script will guide you through setting up contract addresses.
...
After deployment, manually configure the web app:
...
Enter the deployed contract addresses when prompted. The script will create `apps/web/.env.local` with the correct configuration.
